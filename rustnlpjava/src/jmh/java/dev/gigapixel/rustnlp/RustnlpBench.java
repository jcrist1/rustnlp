...
public class RustnlpBench {
    ...
    """;
    static Tok tok = Tok.create();
    static TokRes encode = tok.tokenize(data);
    static String strs = encode.getByIdx(0);
    @Benchmark
    @BenchmarkMode(Mode.AverageTime) // This will report sec/op or ms/op
    public static void benchOpaque(Blackhole bh) {
        var encode = tok.tokenize(data);
        var len = encode.len();
        var tokens = new String[len];
        for (var i = 0; i < len ; i++) {
            var str = encode.getByIdx(i);
            tokens[i] = str;
        }
        bh.consume(encode);
    }
    ...
    public static void main(String[] args) throws RunnerException {
        Options opt = new OptionsBuilder()
                .include(RustnlpBench.class.getSimpleName())
